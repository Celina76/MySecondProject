...
app.factory('FriendService', [
...

		{
			...
			var BASE_URL = "http://localhost:8086/Backend"
			return {
				...
				getMyAcceptedFriends : function() {
					...
					return $http.get(BASE_URL + '/myaccepetedFriends').then(
							function(response) {
								return response.data;
							}, function(errResponse) {
								console.error('Error while fetching Friends');
				...
				accept : function(friend, id) {
					...
					return $http.put(BASE_URL + '/friendaccept/' + friend.id,
							friend).then(function(response) {
						return response.data;
					}, function(errResponse) {
				...
				reject : function(friend, id) {
					...
					return $http.put(BASE_URL + '/friendreject/' + friend.id,
							friend).then(function(response) {
						return response.data;
					}, function(errResponse) {
						console.error('Error while friend user');
				...
				updateFriendRequest : function(friend, id) {
					return $http.put(BASE_URL + '/friend/' + id, friend).then(
							function(response) {
								return response.data;
							}, function(errResponse) {
				...
				deleteFriend : function(id) {
					return $http, delete (BASE_URL + '/friend/' + id).then(
							function(response) {
								return response.data;
							}, function(errResponse) {
