...
app.controller('BlogController', [
		...
		function($scope, BlogService, $location, $rootScope) {
			...
			self.getSelectedBlogComment = function getBlogComment(blogID) {
				...
				BlogService.getBlogComment(blogID).then(function(d) {
					self.blogcomments = d;
			...
			self.fetchAllBlogComments = function() {
				BlogService.fetchAllBlogComments().then(function(d) {
					self.blogcomments = d;
				}, function(errResponse) {
			...
			self.addcomment = function(id, comment, ratingvalue) {
				{
					...
					self
							.commentBlog(self.blogcomment, id, comment,
									ratingvalue);
				}
				self.reset();
			};
			...
			self.commentBlog = function(blogcomment, id, comment, ratingvalue) {
				console.log("comment " + blogcomment.bcomment
						+ " for the blog " + id + " actual comment " + comment)
			...
			self.updateBlog = function(blog, blog_Id) {
				BlogService.updateBlog(blog, blog_Id).then(self.fetchAllBlogs,
						function(errResponse) {
